...
- 日付・時刻はすべて日本時間（JST）で表記する。読者は日本の個人投資家である。海外の指標・イベントも日本時間に換算して書き、必要に応じて「（現地時間◯日）」を括弧で補う。「日本時間」と明記できる箇所は明記する。
- 「注目ニュース」の各項目の見出しは \`### 1. 【重要度：高】イベント名（日付 時刻）\` の形式にし、重要度を必ず 高・中・低 のいずれかで付ける。サイト側でこの表記を読み取り、重要度のバッジとして表示する。見出しの日付・時刻も日本時間とする。
  - 高：FOMC・主要国の政策金利・CPI・雇用統計・GDPなど、株式・為替・金利の全体が動きやすいもの。
  - 中：特定の通貨・セクター・個別企業に影響が出やすいもの（一国の景気指標、主要企業の決算など）。
  - 低：休場・祝日など、直接の値動きより取引環境に関わるもの。
- 重要度の高い項目から順に並べる。
